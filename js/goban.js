    ...
    function update(pos) {
        ...
        if (altMove.checked) {
            if (score.turn == 1) {
                ...
                guideToast(lang == "ja" ? '仮後は黒か白を選びます' : 'Tentative white player choose black or white.')
            }
        ...
        if (altMove.checked) {
            if (score.turn < 3) {
                ...
                $.toast({
                    ...
                    hideAfter: false,
                    stack: false,
                    position: { left : 150, right : 'auto', top : 200, bottom : 'auto' }
                ...
                $("#black").on('click', function () {
                    ...
                    ws.send(b)
                    waitSwapOrNot = false
                    guideToast(lang == "ja" ? '黒を選択しました' : 'You decided that you are a black player.')
                })
                $("#white").on('click', function () {
                    ...
                    ws.send(b)
                    waitSwapOrNot = false
                    guideToast(lang == "ja" ? '白を選択しました' : 'You decided that you are a white player.')
                })
            }
            else {
